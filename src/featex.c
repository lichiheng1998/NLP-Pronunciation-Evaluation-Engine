...
main(int argc, char *argv[])
{
    ...
    if (strcmp(argv[1], "-i") == 0) {
        ...
        if (*(argv[1]+2)) {
            ...
            p = argv[1]+1;
            while (*++p) {
                if (*p == 'u') play |= 1; // utterance
                else if (*p == 'w') play |= 2; // word(s)
                else if (*p == 'p') play |= 4; // phonemes
                else if (*p == 't') play |= 8; // triphones
                else if (*p == 'd') play |= 16; // diphones
                else {
...
#define FPS (16000 / FRATE * 2)
    ...
    // Add words from argument list for alignment
    ...
    // Search alignment with acoustic model
    ...
    while (itor) {
        ...
        while (itor2) {
            ...
            fprintf(stderr, "%s: sub-phone '%s': %.2fs for %.2fs, score %d\n",
                argv[0], mdef->ciname[ae->id.pid.cipid], ae->start / frated,
                ae->duration / frated, ae->score);
            // Store stats of the individual phone
    ...
    for (i = 0; i < n; i++) {
        ...
        if (play & 4) { // play phonemes (default if '-p' specified)
            rawfh = fopen("/tmp/outphone.raw", "wb");
    ...
    for (i = 1; i < n; i++) {
        ...
        // Scan three phonemes at a time
        fprintf(stderr, "%s: triphone %d: %s-%s-%s\n", argv[0], i,
            mdef->ciname[algn[i-1].cipid],
            mdef->ciname[algn[i].cipid],
            mdef->ciname[algn[i+1].cipid]);
        if (play & 8) { // play triphones
            rawfh = fopen("/tmp/outphone.raw", "wb");
            fwrite(obuf, 16000 + (algn[i-1].dur + algn[i].dur +
                                  algn[i+1].dur) * FPS, 1, rawfh);
            fclose(rawfh);
            system("play -q -r16k -ts16 -c1 /tmp/outphone.raw");
            remove("/tmp/outphone.raw");
        }
        ...
        // Define grammar
        grammar[0] = '\0';
        // Note, the intial silence is appended here
        strcat(grammar, "#JSGF V1.0;\ngrammar subalts;\npublic <alts> = sil1 ");
        // Add the first phone in the triphone to the grammar
        ...
        // Add rule into the middle of the triphone in the grammar
        ...
        // Add the third phone in triphone to the grammar
        if (algn[i+1].cipid != mdef->sil) {
             p = mdef->ciname[algn[i+1].cipid];
             q = grammar;
             while (*++q);
        ...
        // Compute n-best score (substitution score)
        nb = ps_nbest(ps);
        j = found = 0;
        target[0] = ' '; target[1] = '\0';
        ...
        *q = '\0';
        ...
        ps_start_utt(ps);
        ps_process_raw(ps, (const int16 *) obuf, 8000 + // samples not bytes
                       (algn[i-1].dur + algn[i].dur) * 160, FALSE, TRUE);
        ps_end_utt(ps);
